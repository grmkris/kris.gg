...
const MAX_PER_SCENE = 2; // diversity cap in the final selection
const MAX_ROUNDS = 5;
...
/** Greedy best-first pick of N, capping near-duplicate scenes. */
function selectDiverse(ordered: Scored[], n: number): Set<string> {
  ...
  // Fill remaining slots ignoring the cap if we came up short.
  if (keep.size < n) {
    for (const s of ordered) {
      if (keep.size >= n) {
        break;
...
async function main(): Promise<void> {
  ...
  const keepCount = flags.keep ? Number(flags.keep) : DEFAULT_KEEP;
  ...
  const keepUuids = selectDiverse(ordered, keepCount);
  for (const s of all) {
